...
import flet as ft
from flet.audio import Audio  # flet-audio==0.84.0
import flet_permission_handler as fph  # flet-permission-handler==0.84.0
from pyradios import RadioBrowser  # pyradios==2.1.1
...
def get_favorites_path():
    """Платформозависимый путь к файлу избранного."""
    if os.name == "posix":  # Android / Linux
        return "/storage/emulated/0/Download/.radio_favorites.json"
    else:
        # Для Windows / macOS – сохраняем в папку приложения (или Documents)
        return os.path.join(os.path.expanduser("~"), ".radio_favorites.json")
...
def save_favorites(favs: list[dict]):
    ...
    try:
        ...
        print(f"Ошибка сохранения избранного: {ex}")
...
async def main(page: ft.Page):
    ...
    # --- Запрос разрешения хранилища (Android) ---
    perm_handler = fph.PermissionHandler()
    try:
        if await perm_handler.check_permission("storage") != fph.PermissionStatus.granted:
            await perm_handler.request_permission("storage")
    except Exception:
        pass  # На десктопе или при отказе – продолжаем
    ...
    def play_station(station: dict):
        ...
        if current_station and current_station.get("url") == station.get("url"):
            # Та же станция – переключаем play/pause
            if audio_player.state == ft.AudioState.PLAYING:
                audio_player.pause()
            else:
                audio_player.resume()
        else:
            ...
            if url:
                ...
                show_snackbar("Нет URL для станции")
    ...
    def update_player(station: dict | None):
        if station is None:
            ...
            if (current_station and current_station.get("stationuuid") == station.get("stationuuid")
                    and audio_player.state == ft.AudioState.PLAYING):
        ...
        content=ft.Row(
            [
                ...
                ft.IconButton(
                    ...
                    icon_size=20,
                    on_click=lambda e: (
                        page.launch_url(current_station.get("homepage", ""))
                ...
                [
                    ...
                    ft.Column(
                        [
                            ...
                            (
                                ft.Text(tags_short, size=10, color="#4b5563", no_wrap=True, max_lines=1)
                                if tags_short
    ...
    def build_category_chip(label: str, tag: str) -> ft.Container:
        ...
        return ft.Container(
            content=ft.Text(
                label, size=12, color="white" if selected else MUTED, weight="bold" if selected else "normal"
            ),
                ...
                content=ft.Column(
                    [
                        ...
                        ft.Container(
                            content=ft.Row(
                                [
                                    ft.Icon(ft.Icons.SEND, color=ACCENT, size=20),
                                    ft.Text("Telegram — @OFFpolice", size=13, expand=True),
                        ...
                        ft.Container(
                            content=ft.Row(
                                [
                                    ft.Icon(ft.Icons.SEND, color=ACCENT, size=20),
                                    ft.Text("X (Twitter) — @OFFpolice2077", size=13, expand=True),
                                    ft.Icon(ft.Icons.OPEN_IN_NEW, color=MUTED, size=16),
                                ],
                                spacing=12,
                            ),
                        ...
                        ft.Container(
                            content=ft.Row(
                                [
                                    ft.Icon(ft.Icons.SEND, color=ACCENT, size=20),
                                    ft.Text("Instagram — @OFFpolice2077", size=13, expand=True),
                                    ft.Icon(ft.Icons.OPEN_IN_NEW, color=MUTED, size=16),
                                ],
                                spacing=12,
                            ),
    ...
    def refresh_current_page():
        if current_page_index[0] == 0:
            fetch_stations(query=search_input.value.strip(), tag=selected_category[0])
        elif current_page_index[0] == 1:
            show_favorites_page()
        page.update()

    # --- Основной макет: шапка, тело, плеер-бар ---
